...
import { getHasilById, HasilLabDetail, PemeriksaanItem, getToken } from "@/lib/api";
...
export default function CetakHasilPage() {
  ...
  // Group pemeriksaans by bidangPeriksa
  const grouped: Record<string, PemeriksaanItem[]> = {};
  for (const p of data.pemeriksaans) {
    if (!grouped[p.bidangPeriksa]) grouped[p.bidangPeriksa] = [];
    grouped[p.bidangPeriksa].push(p);
  }
    ...
    <>
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "24px 0" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "20px" }}>
          <p style={{ fontSize: "14px", fontWeight: "bold", letterSpacing: "1px", textTransform: "uppercase", margin: 0 }}>Hasil Pemeriksaan Laboratorium</p>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px", marginBottom: "16px", border: "1px solid #e2e8f0" }}>
          <tbody>
            {[
              ["No. RM / Kode", data.pasien.kode_registrasi, "Tgl. Registrasi", fmtDate(data.pasien.tanggal_registrasi)],
              ...
              ["Dokter Pengirim", data.pasien.dokter_pengirim || "-", "Tgl. Periksa", fmtDate(data.tanggal_pemeriksaan)],
              ...
              if (typeof row[1] === "object" && row[1] !== null && "colSpan" in (row[1] as object)) {
                ...
                return (
                  <tr key={i} style={{ borderBottom: "1px solid #e2e8f0" }}>
                    <td style={{ padding: "5px 8px", fontWeight: "600", background: "#f8fafc", width: "20%", borderRight: "1px solid #e2e8f0" }}>{row[0]}</td>
              ...
              return (
                <tr key={i} style={{ borderBottom: "1px solid #e2e8f0" }}>
                  <td style={{ padding: "5px 8px", fontWeight: "600", background: "#f8fafc", width: "20%", borderRight: "1px solid #e2e8f0" }}>{row[0]}</td>
                  <td style={{ padding: "5px 8px", width: "30%", borderRight: "1px solid #e2e8f0" }}>{row[1] as string}</td>
                  <td style={{ padding: "5px 8px", fontWeight: "600", background: "#f8fafc", width: "20%", borderRight: "1px solid #e2e8f0" }}>{row[2]}</td>
        ...
        {/* Results grouped by bidang */}
        {Object.entries(grouped).map(([bidang, items]) => (
          <div key={bidang} style={{ marginBottom: "16px" }}>
            <p style={{ fontSize: "11px", fontWeight: "700", color: "#0f766e", textTransform: "uppercase", letterSpacing: "0.5px",
              background: "#f0fdfa", padding: "5px 8px", borderLeft: "3px solid #0f766e", margin: "0 0 0 0" }}>
              {bidang}
            </p>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px", border: "1px solid #e2e8f0", borderTop: "none" }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["Pemeriksaan", "Hasil", "Nilai Normal", "Satuan"].map(h => (
                    <th key={h} style={{ padding: "5px 8px", textAlign: "left", fontWeight: "600", borderBottom: "1px solid #e2e8f0", fontSize: "10px", color: "#64748b" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {items.map((p, i) => (
                  <tr key={p.idPemeriksaan} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "white" : "#fafafa" }}>
                    <td style={{ padding: "5px 8px" }}>{p.subPeriksa}</td>
                    <td style={{ padding: "5px 8px", fontWeight: p.hasilPemeriksaan ? "600" : "normal" }}>
                      {p.hasilPemeriksaan || <span style={{ color: "#94a3b8" }}>—</span>}
                    </td>
                    <td style={{ padding: "5px 8px", color: "#64748b" }}>{p.nilaiNormal || "-"}</td>
                    <td style={{ padding: "5px 8px", color: "#64748b" }}>{p.satuan || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Footer */}
        <div style={{ marginTop: "32px", display: "flex", justifyContent: "space-between", fontSize: "11px", gap: "32px" }}>
          <div style={{ flex: 1 }}>
            <p style={{ color: "#64748b", marginBottom: "8px" }}>Dicetak: {fmtDate(new Date().toISOString())}</p>
            <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: "4px", padding: "8px", fontSize: "10px", color: "#92400e" }}>
              <em>Catatan: Hasil ini hanya berlaku untuk spesimen yang diperiksa.</em>
            </div>
          </div>
          <div style={{ textAlign: "center", minWidth: "140px" }}>
            <p style={{ marginBottom: "40px" }}>Analis Pemeriksa,</p>
            <p style={{ borderTop: "1px solid #1e293b", paddingTop: "4px", fontWeight: "600" }}>{data.siswa?.nama ?? "-"}</p>
            <p style={{ color: "#64748b" }}>Analis Siswa</p>
          </div>
          <div style={{ textAlign: "center", minWidth: "140px" }}>
            <p style={{ marginBottom: "40px" }}>Penanggung Jawab,</p>
            <p style={{ borderTop: "1px solid #1e293b", paddingTop: "4px", fontWeight: "600" }}>( Asesor )</p>
            <p style={{ color: "#64748b" }}>Penanggung Jawab</p>
          </div>
        </div>
      </div>
    </>
